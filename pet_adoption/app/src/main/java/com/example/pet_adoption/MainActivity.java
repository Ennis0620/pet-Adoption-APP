...
public class MainActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        db = FirebaseFirestore.getInstance(); //初始化資料庫
        ...
        if (animal_list != null)   //sharedpreference已經有儲存的物件，直接拿出來使用
        {
            if (search)   //如果使用者是從搜尋頁面按搜尋的話，要先篩選出動物，再顯示出來
            {
                ...
                //傳進firebase
                db.collection("condotion")
                        .add(search_condotion)
    ...
    {
        Context context = MainActivity.this;        //目前是在內部類別(AsyncTask的區塊)，而介面元件是屬於MainActivity的
        ...
        adapter.setViewBinder(new SimpleAdapter.ViewBinder()
        {
            ...
            {
                //目前在AsyncTask類別(Animal) 的onPostExecute方法裡面，已經回到主執行緒
